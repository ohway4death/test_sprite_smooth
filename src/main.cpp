...
// 画像の定義
extern const unsigned short image_data[];
...
void setup(void)
{
  ...
  trump.setColorDepth(8); // カラーモード設定
  trump.createSprite(imageWidth, imageHeight);

  Serial.begin(115200); // シリアルポートを開き，通信速度を19200bpsに設定

  // M5.Lcd.drawBitmap(0, 0, imageWidth, imageHeight, image_data);
  // M5.Lcd.drawJpgFile(SD, "/trump/card_spade_01.jpg");
  ...
  // 無限ループ内でスプライトを作成し、成功するまで繰り返し
  // for (;;)
  // {
  //   sprite_height = (lcd_height + div - 1) / div; // スプライトの高さを設定
  //   bool fail = false;                            // スプライト作成の成否を示す
  //   for (std::uint32_t i = 0; !fail && i < 2; ++i)
  //   {
  //     sprites[i].setColorDepth(lcd.getColorDepth());             // スプライトの色深度を設定
  //     sprites[i].setFont(&fonts::Font2);                         // スプライトのフォントを設定
  //     fail = !sprites[i].createSprite(lcd_width, sprite_height); // スプライトの作成が失敗した場合、True、成功したらFalseになる
  //   }
  //   // 成功した場合はループを抜け、失敗した場合はスプライトを削除し、分割数を増やして再試行
  //   if (!fail)
  //     break;
  ...
  sprites[0].createSprite(320, 80);
  sprites[1].createSprite(320, 80);

  sprite_height = 80;

  // trump.pushImage(0, 0, imageWidth, imageHeight, image_data);

  trump.drawJpgFile(SD, "/trump/card_spade_01.jpg", 0, 0);
  // trump.pushRotateZoom(&sprites[0], 160, 120, 120, 1, 1, 0);
  // sprites[0].pushSprite(&lcd, 0, 0);

  // trump.pushRotateZoom(&sprites[1], 160,  40, 120, 1, 1, 0);
  // sprites[1].pushSprite(&lcd, 0, 80);

  // sprites[0].fillScreen(TFT_BLACK);
  // trump.pushRotateZoom(&sprites[0], 160, -40, 120, 1, 1, 0);
  // sprites[0].pushSprite(&lcd, 0, 160);
...
void loop(void)
{
  ...
  for (int_fast16_t y = 0; y < lcd_height; y += sprite_height)
  {
    ...
    trump.pushRotateZoom(&sprites[flip], 160, center_y-y, round_diff, round_diff/360, round_diff/360);
...
constexpr unsigned short image_data[] = {
